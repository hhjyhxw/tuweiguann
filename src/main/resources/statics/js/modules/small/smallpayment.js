$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'small/smallpayment/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.smallPayment.id == null ? "small/smallpayment/save" : "small/smallpayment/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.smallPayment),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "small/smallpayment/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "small/smallpayment/info/"+id, function(r){
                vm.smallPayment = r.smallPayment;
            });
		},
