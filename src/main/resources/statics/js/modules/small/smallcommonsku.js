$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'small/smallcommonsku/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.smallCommonSku.id == null ? "small/smallcommonsku/save" : "small/smallcommonsku/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.smallCommonSku),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "small/smallcommonsku/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "small/smallcommonsku/info/"+id, function(r){
                vm.smallCommonSku = r.smallCommonSku;
            });
		},
